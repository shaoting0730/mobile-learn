
const api = {
  // 首页
  COVID19Url:'https://v.api.aa1.cn/api/api-yq/index.php',  // 疫情信息
  starUrl:'https://v.api.aa1.cn/api/xingzuo',   // 星座运势
  weatherUrl:'https://v.api.aa1.cn/api/api-tianqi-3/index.php', // 天气
  // 新闻
  newsListUrl:'https://v.api.aa1.cn/api/bilibili-rs/',   // 新闻接口
  // 我的
  QQInfo:'https://v.api.aa1.cn/api/qqjson/index.php',  // 获取QQ信息
  avatarUrl:'https://v.api.aa1.cn/api/api-tx/index.php?wpon=aosijur75fi5huyty5f',  // 随机头像
}

export default api;
